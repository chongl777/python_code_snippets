...
function updateContent(submit, paramsInput) {
    ...
    startLoading();
    $.ajax({
        url: "get_portfolio_lookthrough_by_strategy_scenario_test",
        method: "GET",
        dataType: 'json',
        timeout: 1000*1200,
        data: {
...
var main = function(fund_info, strategies, priorities) {
    ...
    var paramsInput = new (function () {
        ...
        var cached_setup = CachedParams('pf_lookthrough_strategy_scenario_test');

        this.t_date = ko.observable(d3.timeFormat("%Y-%m-%dT%H:%M")(t_date));
        this.ref_date = ko.observable(d3.timeFormat("%Y-%m-%dT%H:%M")(ref_date));
        //this.f_date = ko.observable(cached_setup('f_date', d3.timeFormat("%Y-%m-%dT%H:%M")(t_date)));
        ...
        this.selected_tag = cached_setup('selected_tag', [this.available_tags()[0]]);
