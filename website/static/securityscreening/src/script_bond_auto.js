...
function addTitle(obj, config=true, donwload=false) {
    var source = config?
            '<header class="title"> \
                    <h data-bind="text: title"></h> \
                    <span id="download" \
                     style="background-image: url(\'./static/src/images/button-images.svg\'); \
    ...
    obj.each(function(i, widget) {
        ...
        $('img#config', $html).on('click', function(p) {
...
function updateContent(submit) {
    ...
    startLoading();
    $.ajax({
        url: "daily_screening_report_summary",
        method: "POST",
        dataType: 'json',
        timeout: 1000*120,
        async: false,
...
var paramsInput = new (function () {
    ...
    this.title = ko.pureComputed(function() {
        return "Bond Screening Report (As Of " + self.t_date().slice(0, 10) + ")";
    });
...
function initialize(PARAMS) {
    PARAMS['widget']['security-screening-1'] = addTitle(
        create_screening_table(
            ...
            group_level_1='product_typ',
            group_level_2='None',
            start_expansion_level=1,
            total_expansion_level=null,
            pingate=false), true, false);
    ...
    PARAMS['widget']['security-screening-3'] = addTitle(
        create_screening_table(
            query='div#security-screening-3',
            title='BOTTOM PERFORMERS - 5D',
            datasource='security_screening_3',
            group_level_1='product_typ',
            group_level_2='None',
            start_expansion_level=1,
            total_expansion_level=null,
    ...
    PARAMS['widget']['security-screening-4'] = addTitle(
        create_screening_table(
            query='div#security-screening-4',
            title='TOP PERFORMERS - 5D',
            datasource='security_screening_4',
            group_level_1='product_typ',
            group_level_2='None',
            start_expansion_level=1,
            total_expansion_level=null,
    ...
    PARAMS['widget']['security-screening-0'] = addTitle(
        create_screening_table(
            ...
            group_level_1='bics_industry_level_1',
            group_level_2='None',
            start_expansion_level=0,
            total_expansion_level=1,
            pingate=false), true, false);

    PARAMS['widget']['security-screening-5'] = addTitle(
        create_screening_table(
            query='div#security-screening-5',
            title='Very Distressed Name (Z Score < -1 and Max Draw Down > 50%)',
            datasource='security_screening_5',
            group_level_1='bics_industry_level_1',
            group_level_2='None',
            start_expansion_level=1,
            total_expansion_level=null,
