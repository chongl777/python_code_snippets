(function($){
    $.fn.init_wfi_table = function(options) {
        var self = this;
        self.append('<div class="smart-table-container"></div>');
        var containers = $('div.smart-table-container', self);
        containers.each(function(i, div) {
            initialize.call(div, options);
        });

        self.each(function(i, div) {
            div.options = $('div.smart-table-container', div)[0].options;
            div.open_option_setup = $('div.smart-table-container', div)[0].open_option_setup;
            div.open_download_setup = $('div.smart-table-container', div)[0].open_download_setup;
        });
    ...
    var initialize = function(options) {
        ...
        init_options(self);
        self.options.processOption();

        add_table(self);
        self.update = function() {
            try {
                updateWithData.call(self, clone(self.rawdata), self.options);
            } catch(error) {
                console.log(error);
            };
        };
    ...
    function init_options(self) {
        ...
        self.options['edit_mode_enable'] = ko.observable(false);
        self.options['edit_mode'] = self.options['edit_mode'] || function(x) {};
    ...
    function add_table(self) {
        ...
        function redraw_table(settings, obj) {
            ...
            function add_child_table(row, data) {
                var child_tbl = create_child_table(data.children, data.depth);
                var child_row = $("<tr class=\"child-tbl shown\"></tr>");
                child_row.insertAfter($(row));
                child_row.append('<td></td>');
                $('td', child_row)
                    .attr('colspan', columns_setup.length)
                    .append(child_tbl);
                data.tbl_created = true;
                data.child_row = child_row;
                return child_row
            ...
            api.rows().data().each(function(data, i) {
                if (data.children && data.depth < total_expand) {
                    ...
                    data.child_row = null;
                    if (data.depth < init_expand) {
                        add_child_table(rows[i], data);
                    }
                    $('.clickable', rows[i]).click(
                        function() {
                            var childrow;
                            if (!data.child_row)
                                childrow = add_child_table(rows[i], data);
                            else{
                                childrow = data.child_row;
    ...
    var updateWithData = function(data, options) {
        ...
        data = options.preProcess.call(self, data, options) || data;
    ...
    var updateData = function(data, options) {
        var self = this;
		    ...
		    options.process_child_data.call(self, data, 1);

		    self.tbl.rows().remove().draw();
        data.forEach(function(p) {self.tbl.row.add(p)});
        options.columns_setup.forEach(function(p, i) {
            self.tbl.columns(i).visible(p.visible);
            $(self.tbl.columns(i).header()).html(p.title);
        });
        self.tbl.draw();
        if (options['scrollY'] != '') {
            $('div.dataTables_scrollHeadInner', self.tbl.table().container()).css('width', '100%');
            $('div.dataTables_scrollHeadInner>table', self.tbl.table().container()).css('width', '100%');
        }
    };
