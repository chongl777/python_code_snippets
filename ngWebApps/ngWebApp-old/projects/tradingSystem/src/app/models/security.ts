...
class SecurityImpl {
    ...
    get compositePrice(): number {
        return this.marketData.priceData.compositePrice;
    }
...
export class Security implements Serializable<Security> {
    ...
    deserialize(json: any, securityService?: SecurityService): Security {
        if (this.securityID != (+json['security_id'])) {
            throw Error('Security ID does not match!');
        }

        if (json['product_code'] != null) {
            this._impl = new SECTYPEMAP[json['product_code']](this.marketData).deserialize(json, securityService);
        } else {
