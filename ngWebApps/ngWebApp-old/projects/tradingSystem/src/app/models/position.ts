...
export class Position extends SecurityDataObject implements SerializableAsync<Position> {
    ...
    get diff(): number {
        ...
        let total = Object.values(this.lots).reduce<number>(
            (a: number, b: Lot) => {
                return a + b.security.marketData.priceData.compositeDiff * Math.abs(b.quantity);
            }, 0);
...
export class CashPosition extends Position {
    ...
    get price(): number {
        ...
        let total = Object.values(this.pos).reduce<number>(
            (a: number, b: Position) => {
                return a + b.security.marketData.priceData.compositePrice * b.quantity_abs;
            }, 0);
    ...
    get diff(): number {
        ...
        let total = Object.values(this.pos).reduce<number>(
            (a: number, b: Position) => {
                return a + b.diff * b.quantity_abs;
            }, 0);
