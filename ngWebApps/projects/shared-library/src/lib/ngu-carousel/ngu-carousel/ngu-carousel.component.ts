...
export class NguCarousel<T> extends NguCarouselStore
    ...
    private _inputValidation() {
        ...
        if (this.inputs.grid.xxl === undefined) {
            this.inputs.grid.xxl = this.inputs.grid.xl;
        }
    ...
    private _storeCarouselData(): void {
        ...
        this.deviceWidth = isPlatformBrowser(this.platformId) ? window.innerWidth : breakpoints.xxl;

        this.carouselWidth = this.carouselMain1.nativeElement.offsetWidth;

        if (this.type === 'responsive') {
            this.deviceType =
                this.deviceWidth >= breakpoints.xxl
                    ? 'xxl'
    ...
    private _carouselSize(): void {
        ...
        if (this.vertical.enabled) {
            ...
            const itemWidthXXL = `${this.styleid} > .item {height: ${this.vertical.height /
                +this.inputs.grid.xxl}px}`;
            ...
            itemStyle = `@media (max-width:${breakpoints.sm - 1}px){${itemWidthXS}}
                    ...
                    @media (min-width:${breakpoints.xl}px){${itemWidthXXL}}`;
            ...
            const itemWidthXXL = `${this.styleid} > .item {flex: 0 0 ${100 /
                +this.inputs.grid.xxl}%; width: ${100 / +this.inputs.grid.xxl}%}`;
            ...
            itemStyle = `@media (max-width:${breakpoints.sm - 1}px){${itemWidthXS}}
                    ...
                    @media (min-width:${breakpoints.lg}px){${itemWidthLG}}
                    @media (min-width:${breakpoints.xl}px){${itemWidthXL}}
                    @media (min-width:${breakpoints.xxl}px){${itemWidthXXL}}`;
        } else {
    ...
    private _transformStyle(slide: number): void {
      ...
      @media (min-width: ${breakpoints.xxl}px) {${this._transformString('xxl', slide)} }`;
        } else {
