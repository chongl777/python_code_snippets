...
export class TicketDetailsComponent implements OnInit {
    ...
    private updateCounterpties(): void {
        if (this.ticket == null) {
            return;
        }
        this.availableCounterpties = {}
        Object.values(
            this.tradeTicketsService.counterparties)
            .filter(x => x.productCode == this.ticket.security.productCode)
            .forEach(x => {
                this.availableCounterpties[x.Alias] = x;
            });

    }

    get availableCounterptiesArray(): any[] {
        return Object.values(this.availableCounterpties);
    }
