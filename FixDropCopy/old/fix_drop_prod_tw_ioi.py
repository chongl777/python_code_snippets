...
def handle_msg_factory(logger):
    ...
    return handle_msg_async


def send_ioi_async(fix_engine, ioi):
    logger = fix_engine.logger
    subject_tmplt = """
...
def main():
    ...
    logger = getLogger(
        'fix_engine',
        '%(asctime)s %(name)-12s %(threadName)s: %(funcName)-20s'
    ...
    with DropFIX(
            ...
            handle_msg=handle_msg_factory(logger),
    ) as client_engine:
        try:
            logger1 = getLogger(
                'ioi_con',
            ...
            client_engine.client.add_connection(
                name='ioi_con',
                host='wfiubuntu01.wfi.local', port=6004, protocol=protocol,
