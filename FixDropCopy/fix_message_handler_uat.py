...
from fixlibs.fix_handlers import (
    MQServerMixIn, route, broadcast, strftime, broadcast_bigsize,
    to_datetime, to_date, JSON)
...
from fixlibs.watchlist_utils import (
    send_watchlist_matched_notification)
...
class FIXMessageHandler(FIXOrderHandlerMixIn, FIXIOIHandlerMixIn,
                        MQServerMixIn):
    def __init__(
            self, mq_queue,
        ...
        # self.process_orders(
        #     fix_dbutils.load_today_orders(
        #         self, t_date, 'WESTFIXDEMO', 'TRADEWEB', engine))

        # t_date = pd.to_datetime('2020-09-11')

        self.process_iois(
            fix_dbutils.load_today_iois(t_date, engine))

        self.load_securities_info()
        self.debug = True
    ...
    @broadcast_bigsize(route='broadcast_watchlist',
                       exchange='trading_signal_exchange')
...
def main():
    logger = getLogger(
        ...
        ' %(funcName)-17s: %(message)s (on %(filename)s)'
    )
