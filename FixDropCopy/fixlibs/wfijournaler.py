...
class WFIJournalerClient(Journaler):
    ...
    def cancelTxn(self, session, targetCompId, senderCompId, txn):
        s = self.session_factory()
        try:
            txn = self.__getTxn(
    ...
    def correctTxn(self, session, targetCompId, senderCompId, txn):
        try:
            self.cancelTxn(session, targetCompId, senderCompId, txn)
            self.handleTxn(session, targetCompId, senderCompId, txn)
        except Exception as e:
            self.logger.exception(e)
            raise(e)

    def handleReject(self, session, targetCompId, senderCompId, execID):
