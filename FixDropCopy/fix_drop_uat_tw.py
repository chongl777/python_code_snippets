        ...
        try:
            ...
            try:
                ...
                title = t_date.strftime(
                    '[FIX Handle Message Error] %Y-%m-%d, handler: ') + \
                    handler.__name__
...
def send_ioi_async(fix_engine, session, ioi):
    subject_tmplt = """
...
class DropFIX(FixMqEngine):
    ...
    def responseIOI(self, ioi_info, resp_info):
        ...
        msg.setField(tags.Side,
                     {'1': '2', '2': '1'}[str(ioi_info['Side'])])
        ...
        # msg.setField(tags.PartyID, 'chong.liu@westfiledinvestment.com')

        party = FIXContext()
        party.setField(tags.PartyID, "chong.liu@westfieldinvestment.com")
    ...
    def handle_ioi(self, connectionHandler, msg):
        try:
            import pdb; pdb.set_trace()
            ioi = FixMqEngine.handle_ioi(self, connectionHandler, ioi)
        except Exception as e:
            self.__logger.excpetion(e)
...
def main():
    ...
    with DropFIX(
            ...
            senderCompId="WESTFIXDEMO",
            protocol=protocol,
            journaler=WFIJournalerClient(engine, logger),
            mq_queue='trade_web_uat',
