...
def _value_convertible(tmat,
    ...
    # Su = treeConvBondValue[2, 2]
    # S = treeConvBondValue[2, 1]
    # Sl = treeConvBondValue[2, 0]
    # dxu = treeStockValue[2, 2] - treeStockValue[2, 1]
    # dxl = treeStockValue[2, 1] - treeStockValue[2, 0]
    # gamma = (2/dxl * Sl + 2/dxu * Su - (2/dxl + 2/dxu)*S) / (dxl + dxu)

    deltaUp = (treeConvBondValue[2, 2] - treeConvBondValue[2, 1]) / \
              (treeStockValue[2, 2] - treeStockValue[2, 1])
    deltaDn = (treeConvBondValue[2, 1] - treeConvBondValue[2, 0]) / \
        (treeStockValue[2, 1] - treeStockValue[2, 0])

    gamma = (deltaUp - deltaDn) / (treeStockValue[2, 2] - treeStockValue[2, 0]) * 2

    # import pdb; pdb.set_trace()
    # deltaUp1 = (treeConvBondValue[2, 3] - treeConvBondValue[2, 2]) / \
    #           (treeStockValue[2, 3] - treeStockValue[2, 2])
    # deltaDn1 = (treeConvBondValue[2, 2] - treeConvBondValue[2, 1]) / \
    #           (treeStockValue[2, 2] - treeStockValue[2, 1])

    # gamma1 = (deltaUp1 - deltaDn1) / (treeStockValue[1, 1] - treeStockValue[1, 0])
